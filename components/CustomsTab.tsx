...
import { customsData, CustomItem } from '../data/customs_data';
...
export default function CustomsTab({ captureTarget }: CustomsTabProps) {
  const formatForStudio = (item: CustomItem) => {
    let text = `🏺 COSTUME BÍBLICO: ${item.title}\n`;
    if (item.biblicalReference) text += `📖 Base Bíblica: ${item.biblicalReference}\n`;
    if (item.description) text += `📜 Descrição: ${item.description}\n`;
    if (item.details?.length) text += `🔍 Detalhes Culturais:\n- ${item.details.join('\n- ')}\n`;
    if (item.spiritualMeaning) text += `✨ Significado Espiritual: ${item.spiritualMeaning}\n`;
    if (item.throneRevelation) text += `👑 Revelação Trono: ${item.throneRevelation}\n`;
    return text;
  };

  const createProject = async (item: CustomItem, type: 'sermon' | 'lesson') => {
    const newSermon: Sermon = {
      id: `custom_proj_${Date.now()}`,
      title: type === 'sermon' ? `Sermão: O Significado de ${item.title}` : `Aula: Costumes Bíblicos - ${item.title}`,
      type: type === 'sermon' ? 'expository' : 'bible-class',
      date: new Date().toISOString(),
      tags: ['Costumes', 'Cultura Bíblica', 'Teologia'],
      status: 'draft',
      version: 1,
      folder: type === 'sermon' ? 'Sermões Temáticos' : 'Cultura Bíblica',
      blocks: type === 'sermon' ? [
        { id: 'b1', type: 'context', title: 'Contexto Cultural', content: item.description || '' },
        { id: 'b2', type: 'text-base', title: 'Texto Base', content: item.biblicalReference || '' },
        { id: 'b3', type: 'point', title: 'Detalhes do Costume', content: (item.details || []).join('\n') },
        { id: 'b4', type: 'point', title: 'Significado Espiritual', content: item.spiritualMeaning || '' },
        { id: 'b5', type: 'application', title: 'Revelação Profunda', content: item.throneRevelation || '' }
      ] : [
        { id: 'b1', type: 'chronology', title: 'Referência', content: item.biblicalReference },
        { id: 'b2', type: 'point', title: 'O Costume na Prática', content: `${item.description}\n\n${(item.details || []).join('\n')}` },
        { id: 'b3', type: 'exegesis', title: 'Significado', content: item.spiritualMeaning || '' },
        { id: 'b4', type: 'student-questions', title: 'Questões para Classe', content: `1. Como esse costume nos ajuda a entender melhor a Bíblia?\n2. Qual o ensino prático de ${item.title} para nós hoje?` }
      ]
    };
    ...
    <div className="p-6 max-w-5xl mx-auto space-y-12">
      <div className="text-center space-y-4 mb-12">
        <h1 className="text-4xl font-black tracking-tight text-slate-900 dark:text-white flex items-center justify-center gap-4">
          <div className="w-12 h-12 md:w-16 md:h-16 rounded-2xl shadow-lg flex items-center justify-center text-3xl md:text-4xl bg-amber-100 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-800">
            🕎
          </div>
          Costumes Judaicos
        </h1>
        <p className="text-lg text-slate-600 dark:text-slate-400 max-w-2xl mx-auto">
          Mergulhe na vida diária do tempo bíblico e descubra os tesouros escondidos por trás das práticas culturais do povo de Deus.
        </p>
      </div>

      {customsData.map((category) => (
            ...
            {category.items.map((item) => (
              ...
              >
                ...
                <div className="pr-12 flex items-start gap-4">
                  <div className={`w-14 h-14 md:w-16 md:h-16 rounded-xl shadow-sm hidden sm:flex items-center justify-center text-3xl ${colorMap[item.color] || 'bg-slate-100 dark:bg-slate-800'}`}>
                    {item.icon || '📜'}
                  </div>
                  <div>
                    ...
                    <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mt-1">
                      {item.biblicalReference && `📖 ${item.biblicalReference}`}
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-6 space-y-4">
                  {item.description && (
                    <div>
                      <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-1">Descrição</h4>
                  ...
                  {item.spiritualMeaning && (
                    <div className="bg-emerald-50 dark:bg-emerald-900/20 p-3 rounded-lg border border-emerald-100 dark:border-emerald-900/30">
                      ...
                      <p className="text-sm text-emerald-800 dark:text-emerald-300">{item.spiritualMeaning}</p>
                    </div>
                  )}
                    ...
                    <button 
                      onClick={() => {
                        window.dispatchEvent(new CustomEvent('dabar-play-tts', {
                          detail: { title: item.title, subtitle: 'Costumes', text: formatForStudio(item) }
                        }));
                      }}
