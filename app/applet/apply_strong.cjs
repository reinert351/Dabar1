...
function run() {
  ...
  try {
    ...
    console.error("JSON parse error for TS:", e.message);
    return;
  }
  
  let ptData;
  try {
      ptData = JSON.parse(fs.readFileSync('strong_pt.json', 'utf8'));
  } catch(e) {
      console.error("JSON parse error for PT json:", e.message);
      return;
  }
